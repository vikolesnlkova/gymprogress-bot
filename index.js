...
const {
  BOT_TOKEN,
  USE_WEBHOOK = 'true',               // 'true' для Koyeb (webhook), 'false' для локальной отладки (polling)
  PORT = 3000,
  WEBHOOK_DOMAIN,                     // например: https://your-app.koyeb.app
  WEBHOOK_SECRET = 'please-change-me' // длинная случайная строка
} = process.env;

if (!BOT_TOKEN) {
  console.error('❌ Не задан BOT_TOKEN'); process.exit(1);
}
...
// ---------------------- ИНИЦИАЛИЗАЦИЯ БОТА ----------------------
if (USE_WEBHOOK === 'true') {
  // WEBHOOK-МОД (Koyeb/хостинг)
  bot = new TelegramBot(BOT_TOKEN, { polling: false });
  app = express();
  app.use(express.json()); // нужно для bot.processUpdate

  const path = `/telegram/${WEBHOOK_SECRET}`;
  if (!WEBHOOK_DOMAIN) {
    console.error('❌ Не задан WEBHOOK_DOMAIN (например, https://<app>.koyeb.app)');
    process.exit(1);
  }
  ...
  bot.setWebHook(webhookUrl)
    ...
    .catch(err => {
      console.error('❌ Ошибка установки вебхука:', err?.message || err);
      process.exit(1);
    });
  ...
  // POLLING-МОД (локальная отладка)
  bot = new TelegramBot(BOT_TOKEN, { polling: true });
  console.log('✅ Бот запущен в режиме polling (локальная отладка)');
}

// ---------------------- ДАЛЕЕ — ВЕСЬ ТВОЙ ФУНКЦИОНАЛ ----------------------
...
function calculateRecommendedWeights(user) {
  ...
  switch(experience) {
    case 'beginner': baseMultiplier = 0.4; assistanceMultiplier = 0.3; break;
    case 'intermediate': baseMultiplier = 0.6; assistanceMultiplier = 0.45; break;
    case 'advanced': baseMultiplier = 0.8; assistanceMultiplier = 0.6; break;
    default: baseMultiplier = 0.5; assistanceMultiplier = 0.35;
  }
  switch(goal) {
    case 'weight_loss': baseMultiplier *= 0.9; assistanceMultiplier *= 0.85; break;
    case 'strength': baseMultiplier *= 1.1; assistanceMultiplier *= 1.0; break;
    case 'muscle_gain': baseMultiplier *= 1.0; assistanceMultiplier *= 0.95; break;
    case 'endurance': baseMultiplier *= 0.8; assistanceMultiplier *= 0.7; break;
  }
...
function recommendWeightForExercise(user, exerciseName) {
  ...
  if (exercise.includes('жим') || exercise.includes('присед') || exercise.includes('тяга') || exercise.includes('становая')) {
...
function parseTimeInput(input) {
  ...
  if (!minutesMatch && !secondsMatch && /[a-zA-Z]/.test(input)) return null;
...
function startTimer(chatId, userId, exerciseName, duration) {
  ...
  activeTimers.set(timerId, {
    userId, chatId, exerciseName, duration, startTime: Date.now(), timeoutId
  });
  if (duration > 30) {
    setTimeout(() => {
...
bot.on('message', (msg) => {
  ...
  if ([
    ...
    '❌ Отмена','➕ Добавить еще упражнение','🗑 Удалить последнее упражнение',
    '✅ Завершить создание','⏱️ Таймер упражнений'
  ...
  if (user.state === 'questionnaire_weight') {
    ...
    bot.sendMessage(chatId,
      ...
      { reply_markup: { keyboard: [
        ['💪 Набор массы'],['🏃 Похудение'],['🏋️ Увеличение силы'],
        ['🏃‍♂️ Выносливость'],['🧘 Поддержание формы'],['❌ Отмена']
      ], resize_keyboard: true } }
    );
    return;
  }
  ...
  if (user.state === 'questionnaire_goal') {
    ...
    if (goal) {
      ...
      bot.sendMessage(chatId,
        ...
        { reply_markup: { keyboard: [
          ['🚀 Начинающий'],['📈 Средний'],['💎 Продвинутый'],['❌ Отмена']
        ], resize_keyboard: true } }
      );
    }
    return;
  ...
  if (user.state === 'questionnaire_experience') {
    const experiences = {'🚀 начинающий': 'beginner','📈 средний': 'intermediate','💎 продвинутый': 'advanced'};
  ...
  if (user.state === 'creating_exercises') {
    ...
    if (currentExercise.step === 'name') {
      ...
      const weight = parseFloat(text);
      if (isNaN(weight)) return bot.sendMessage(chatId, '❌ Введите число для веса!');
      ...
      if (isNaN(sets) || sets < 1) return bot.sendMessage(chatId, '❌ Введите число больше 0 для подходов!');
      currentExercise.sets = sets;
      user.newWorkout.exercises.push({ name: currentExercise.name, weight: currentExercise.weight, sets: currentExercise.sets });
      ...
      if (user.newWorkout.exercises.length > 0) keyboard.push(['🗑 Удалить последнее упражнение']);
      keyboard.push(['✅ Завершить создание']);
      bot.sendMessage(chatId, message, { reply_markup: { keyboard, resize_keyboard: true } });
      user.newWorkout.currentExercise = { step: 'name' };
    }
    return;
  }
...
bot.onText(/➕ Создать тренировку/, (msg) => {
  ...
  if (!userData.get(userId)) {
    userData.set(userId, { workouts: [], state: 'creating_name', profile: {} });
  } else {
...
bot.onText(/📋 Мои тренировки/, (msg) => {
  const chatId = msg.chat.id;
  const userId = msg.from.id;
  const user = userData.get(userId) || { workouts: [] };
  if (!user.workouts.length) return bot.sendMessage(chatId, '❌ У вас пока нет тренировок. Создайте первую!');
...
bot.onText(/🏋️ Начать тренировку/, (msg) => {
  ...
  const userId = msg.from.id;
  const user = userData.get(userId);
  if (!user || !user.workouts?.length) return bot.sendMessage(chatId, '❌ У вас пока нет тренировок. Сначала создайте тренировку!');
  const buttons = user.workouts.map((workout, index) => [{ text: workout.name, callback_data: 'select_workout_' + index }]);
  bot.sendMessage(chatId, '🏋️ Выберите тренировку:', { reply_markup: { inline_keyboard: buttons } });
});
...
bot.onText(/⏱️ Таймер упражнений/, (msg) => {
  ...
  const userId = msg.from.id;
  if (!userData.get(userId)) {
    userData.set(userId, { workouts: [], state: 'setting_timer', profile: {} });
  } else {
...
bot.onText(/❌ Отмена/, (msg) => {
  ...
  const userId = msg.from.id;
  const user = userData.get(userId);
  if (!user) return showMainMenu(chatId, msg.from.first_name, 'Действие отменено.');
  ...
  if (cancelledTimers > 0) bot.sendMessage(chatId, `⏹️ Отменено активных таймеров: ${cancelledTimers}`);
  user.state = 'menu';
  delete user.timerData;
...
bot.onText(/✅ Завершить создание/, (msg) => {
  ...
  if (user?.state === 'creating_exercises') {
    ...
    bot.sendMessage(chatId,
      ...
      { reply_markup: { keyboard: [
        ['🏋️ Начать тренировку'],['➕ Создать тренировку'],
        ['📋 Мои тренировки'],['⏱️ Таймер упражнений']
      ], resize_keyboard: true } }
    );
  }
});
...
bot.on('callback_query', (callbackQuery) => {
  ...
  if (data.startsWith('select_workout_')) {
    const workoutIndex = parseInt(data.split('_')[2], 10);
    ...
    user.currentWorkout = {
      ...
      exercises: workout.exercises.map(ex => ({
        name: ex.name, weight: ex.weight, targetSets: ex.sets, completedSets: []
      }))
    };
    ...
    currentExercise.completedSets.push({ number: currentExercise.completedSets.length + 1, weight: currentExercise.weight, reps: 8 });
    showExercise(chatId, userId);
...
function showExercise(chatId, userId) {
  ...
  const buttons = [];
  if (!allSetsCompleted) buttons.push([{ text: '✅ Подход выполнен', callback_data: 'set_done_' + exerciseIndex }]);
  if (allSetsCompleted && !isLastExercise) buttons.push([{ text: '➡️ Следующее упражнение', callback_data: 'next_exercise' }]);
  ...
  if (!workout.currentMessageId) {
    bot.sendMessage(chatId, message, { reply_markup: { inline_keyboard: buttons } })
      .then(sentMsg => { workout.currentMessageId = sentMsg.message_id; });
  } else {
    bot.editMessageText(message, { chat_id: chatId, message_id: workout.currentMessageId, reply_markup: { inline_keyboard: buttons } });
  }
}
...
function finishWorkout(chatId, userId) {
  ...
  message += completedSets >= totalSets ? '💪 Отличная работа!' : '💪 Хорошая работа, в следующий раз сделаете больше!';
