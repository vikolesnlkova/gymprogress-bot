...
bot.on('message', (msg) => {
  ...
  // Обработка создания тренировки - название
  ...
  if (user.state === 'creating_exercises') {
    ...
    if (currentExercise.step === 'name') {
      ...
      currentExercise.step = 'reps';
      
      bot.sendMessage(chatId, message + 
        '\n\n🎯 Теперь введите количество повторений в подходе:\n' +
        'Пример: "8" или "12-15" для диапазона',
        {
          reply_markup: {
            keyboard: [['❌ Отмена']],
            resize_keyboard: true
    ...
    else if (currentExercise.step === 'reps') {
      const reps = text;
      if (!reps.match(/^\d+([-–]\d+)?$/)) {
        return bot.sendMessage(chatId, '❌ Введите число или диапазон повторений (например: "8" или "12-15"):');
      }
      ...
      message += '🔄 Повторений: ' + currentExercise.reps + '\n';
      ...
      user.newWorkout.exercises.forEach((ex, i) => {
        message += (i + 1) + '. ' + ex.name + ' - ' + ex.weight + 'kg × ' + ex.reps + ' (' + ex.sets + ' подходов)\n';
      });
  ...
  // Если сообщение не обработано и есть кнопки меню
  if ([
    '🏋️ Начать тренировку', 
    '➕ Создать тренировку', 
    '📋 Мои тренировки',
...
// Обработчики кнопок целей тренировок
bot.onText(/💪 Набор массы/, (msg) => {
  const chatId = msg.chat.id;
  const userId = msg.from.id;
  const user = userData.get(userId);
  
  if (user && user.state === 'questionnaire_goal') {
...
bot.onText(/🏋️ Увеличение силы/, (msg) => {
  const chatId = msg.chat.id;
  const userId = msg.from.id;
  const user = userData.get(userId);
  
  if (user && user.state === 'questionnaire_goal') {
    user.profile.goal = 'strength';
    user.state = 'questionnaire_experience';
...
bot.onText(/🧘 Поддержание формы/, (msg) => {
  const chatId = msg.chat.id;
  const userId = msg.from.id;
  const user = userData.get(userId);
...
// Обработчики кнопок опыта
bot.onText(/🚀 Начинающий/, (msg) => {
  const chatId = msg.chat.id;
  const userId = msg.from.id;
  const userName = msg.from.first_name || 'друг';
  const user = userData.get(userId);
  
  if (user && user.state === 'questionnaire_experience') {
    ...
    message += `📊 Ваш профиль:\n`;
    message += `• Пол: ${getGenderText(user.profile.gender)}\n`;
    ...
    showMainMenu(chatId, userName, message);
  }
});

bot.onText(/📈 Средний/, (msg) => {
  const chatId = msg.chat.id;
  const userId = msg.from.id;
  const userName = msg.from.first_name || 'друг';
  const user = userData.get(userId);
...
bot.onText(/➕ Создать тренировку/, (msg) => {
  ...
  user.state = 'creating_name';
  user.newWorkout = { exercises: [] };
  
  bot.sendMessage(chatId, 
    `📝 ${userName}, введите название тренировки:\n\n` +
...
bot.onText(/📋 Мои тренировки/, (msg) => {
  ...
  user.workouts.forEach((workout, index) => {
    ...
    workout.exercises.forEach((ex, exIndex) => {
      message += `   ${exIndex + 1}. ${ex.name} - ${ex.weight}kg × ${ex.reps} (${ex.sets} подходов)\n`;
    });
    message += '\n';
  });
...
bot.onText(/⏱️ Таймер упражнений/, (msg) => {
  ...
  user.state = 'setting_timer';
  
  bot.sendMessage(chatId,
    `⏱️ ${userName}, установите таймер для упражнения\n\n` +
...
bot.onText(/❌ Отмена/, (msg) => {
  ...
  showMainMenu(chatId, userName, 'Действие отменено.');
});
...
bot.onText(/➕ Добавить еще упражнение/, (msg) => {
  ...
  if (user && user.state === 'creating_exercises') {
    user.newWorkout.currentExercise = { step: 'name' };
    
    bot.sendMessage(chatId,
      `💪 ${userName}, введите название упражнения:\n\n` +
      'Пример: "Жим штанги лежа"',
      {
        reply_markup: {
          keyboard: [['❌ Отмена']],
...
bot.onText(/🗑 Удалить последнее упражнение/, (msg) => {
  ...
  if (user && user.state === 'creating_exercises' && user.newWorkout.exercises.length > 0) {
...
function showExercise(chatId, userId) {
  ...
  message += `🔄 Количество повторений: ${exercise.reps}\n`;
  ...
  if (exercise.completedSets.length > 0) {
    ...
    exercise.completedSets.forEach(set => {
      message += `   ${set.number}. ${set.weight}kg × ${set.reps}\n`;
    });
  }
...
function finishWorkout(chatId, userId) {
  const user = userData.get(userId);
  const userName = user.profile.name || userData.get(userId)?.profile?.name || 'друг';
  const workout = user.currentWorkout;
  ...
  let message = `🎉 ${userName}, тренировка завершена!\n\n`;
  message += '📊 Итоги:\n';
